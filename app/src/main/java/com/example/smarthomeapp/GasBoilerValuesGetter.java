...
public class GasBoilerValuesGetter extends AsyncTask<String,Void,String> {
    ...
    public static List<String> CurrentTempList = new ArrayList<>();
    public static List<String> StatusList = new ArrayList<>();
    public static List<String> TemperatureRangeList = new ArrayList<>();
    public static List<String> SetpointSourceList = new ArrayList<>();
    ...
    public GasBoilerValuesGetter(Context ctx){
        context = ctx;
    }
    ...
    protected void onPreExecute() {
        SetpointTempList.clear();
        CurrentTempList.clear();
        StatusList.clear();
        TemperatureRangeList.clear();
        SetpointSourceList.clear();
        TimestampList.clear();
...
//            DisplayWeathForecast();
    ...
    public void FillAllParams(String result)
    {
        ...
        while (i < result.length())
        {
            ...
            SetpointTempList.add(id-1, Result);
            i++;
            k = i;
            //CurrentTemp
            while(result.charAt(i) != '|')
            ...
            Result = result.substring(k,i);
            CurrentTempList.add(id-1, Result);
            i++;
            k = i;
            //Status
            while(result.charAt(i) != '|')
            ...
            Result = result.substring(k,i);
            TemperatureRangeList.add(id-1, String.valueOf(Float.parseFloat(Result)));
            i++;
            k = i;
            //SetpointSource
            while(result.charAt(i) != '|')
            ...
            Result = result.substring(k,i);
            SetpointSourceList.add(id-1, Result);
            i++;
            k = i;
            //Timestamp
            while(result.charAt(i) != '|')
